...
import { exportDirectoryUsers } from '@/lib/zhiban/admin-data';
...
export async function GET(r: NextRequest) {
  try {
    const p = await requireRequestPermission('account:read'),
      ...
      b = await exportDirectoryUsers(getZhibanPool(), p, {
    ...
    return new NextResponse(new Uint8Array(b), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename*=UTF-8''${encodeURIComponent('用户信息.xlsx')}`,
      },
    });
  } catch (e) {
    return (
