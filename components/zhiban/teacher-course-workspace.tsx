    ...
    <div className="min-h-screen bg-[#f1f5fb] text-slate-800">
      ...
      <header className="border-b bg-white px-5 py-4 md:px-8">
        <div className="mx-auto flex max-w-[1500px] flex-wrap items-center justify-between gap-4">
          ...
          <Link
            href={`/zhiban/teacher/courses/${course.id}/settings`}
      ...
      <div className="mx-auto flex max-w-[1580px]">
        <aside className="hidden w-60 shrink-0 border-r bg-white py-4 lg:block">
          <WorkspaceNav href={`/zhiban/teacher/courses/${course.id}`} icon={LayoutDashboard} active>
            课程首页
          </WorkspaceNav>
          <WorkspaceNav href={`/zhiban/teacher/classrooms${query}`} icon={BookOpen}>
            章节与课堂
          </WorkspaceNav>
          <WorkspaceNav href={`/zhiban/teacher/pbl${query}`} icon={Workflow}>
            PBL 项目
          </WorkspaceNav>
          <WorkspaceNav href={`/zhiban/teacher/grades${query}`} icon={ClipboardCheck}>
            学生成绩
          </WorkspaceNav>
          <WorkspaceNav href={`/zhiban/teacher/profiles${query}`} icon={BarChart3}>
            学习分析
          </WorkspaceNav>
          <WorkspaceNav href={`/zhiban/teacher/risks${query}`} icon={AlertTriangle}>
            风险预警
          </WorkspaceNav>
          <WorkspaceNav href={`/zhiban/teacher/agents${query}`} icon={Bot}>
            智能体中心
          </WorkspaceNav>
          <div className="my-3 border-t" />
          <WorkspaceNav href={`/zhiban/teacher/courses/${course.id}/settings`} icon={Settings}>
            课程设置
          </WorkspaceNav>
        </aside>
        ...
        <main className="min-w-0 flex-1 p-5 md:p-8">
          <section className="mb-5 grid gap-4 md:grid-cols-4">
            ...
            <Summary
              ...
              suffix="项资源"
            />
            <Summary icon={Bot} label="启用智能体" value={enabledAgents(course)} suffix="个角色" />
          </section>
          ...
          <section className="border bg-white p-5">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b pb-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  从一个课程上下文进入课堂、项目、成绩和学习支持模块
                </p>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <span>类型：</span>
                <button className="flex items-center gap-2 rounded border px-3 py-2">
                  全部
            ...
            <div className="space-y-3">
              {features.map(({ label, description, icon: Icon, route }) => (
                <Link
                  key={route}
                  href={`/zhiban/teacher/${route}${query}`}
                  className="group flex items-center gap-4 rounded bg-[#f6f7f9] px-5 py-4 transition hover:bg-blue-50"
                >
...
function enabledAgents(course: TeacherCourse) {
  return Object.values(course.agentSettings).filter(Boolean).length;
}
