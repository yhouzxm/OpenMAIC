    ...
    <div className="min-h-[calc(100vh-52px)] bg-[#f1f5fb]">
        ...
        <div className="mx-auto max-w-[1500px]">
          ...
          <div className="flex flex-col gap-7 lg:flex-row lg:items-center">
            ...
            <div className="flex w-full flex-wrap items-center gap-3 rounded-lg bg-white/35 p-3 backdrop-blur-sm sm:flex-nowrap sm:gap-4 sm:p-4 lg:w-[430px]">
              ...
              <Link href={`/zhiban/student/courses/${courseId}`} className="shrink-0 rounded bg-[#1677e8] px-4 py-2 text-sm hover:bg-blue-700">
                {progress ? '继续学习' : '开始学习'}
              </Link>
            </div>
          </div>
      ...
      <div className="mx-auto flex max-w-[1580px] items-stretch">
        <aside className="sticky top-[52px] hidden h-[calc(100vh-52px)] w-60 shrink-0 self-start overflow-y-auto border-r bg-white py-4 lg:block">
          <CourseNav href={`/zhiban/student/courses/${courseId}`} icon={BookOpen}>目录</CourseNav>
          <CourseNav href={`/zhiban/student/courses/${courseId}/introduction`} icon={FileText}>简介</CourseNav>
          <CourseNav href={`/zhiban/student/courses/${courseId}/tools`} icon={Bot}>学习工具</CourseNav>
          <CourseNav href={`/zhiban/student/courses/${courseId}/coursework`} icon={MessagesSquare}>讨论与作业</CourseNav>
          <CourseNav href={`/zhiban/student/courses/${courseId}/resources`} icon={FileText}>课程资源</CourseNav>
          <div className="my-3 border-t" />
          <CourseNav href={`/zhiban/student/courses/${courseId}/grades`} icon={GraduationCap}>学习成绩</CourseNav>
          <CourseNav href={`/zhiban/student/courses/${courseId}/analysis`} icon={BarChart3}>学习分析</CourseNav>
          <CourseNav href={`/zhiban/student/courses/${courseId}/support`} icon={ClipboardCheck}>学习支持</CourseNav>
        </aside>
        <main className="min-w-0 max-w-full flex-1 overflow-x-hidden p-2 sm:p-4 md:p-6">{children}</main>
      </div>
    </div>
  );
}

function CourseNav({ href, icon: Icon, children }: { href: string; icon: typeof BookOpen; children: React.ReactNode }) {
  return <Link href={href} className="flex items-center gap-3 border-l-4 border-transparent px-7 py-3.5 text-sm hover:border-[#1677e8] hover:bg-blue-50 hover:text-[#1677e8]"><Icon className="size-4" />{children}</Link>;
}

function dateRange(items: ZhibanCourseClassroom[]) {
  const starts = items.map((item) => item.opensAt).filter((value): value is string => Boolean(value)).sort();
  const ends = items.map((item) => item.closesAt).filter((value): value is string => Boolean(value)).sort();
  const format = (value?: string) => value ? new Date(value).toLocaleDateString('zh-CN') : '未设置';
