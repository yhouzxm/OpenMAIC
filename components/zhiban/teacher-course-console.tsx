...
const isoDate = (value: FormDataEntryValue | undefined) =>
...
export function TeacherCourseConsole({ principalName }: { principalName: string }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setSelectedId((id) =>
        result.courses.some((course) => course.id === id) ? id : (result.courses[0]?.id ?? ''),
      );
    } catch (error) {
    ...
    try {
      await api(`/api/zhiban/teacher/courses/${course.id}`, {
        ...
        body: JSON.stringify({
          ...
          startsAt: isoDate(values.startsAt),
          endsAt: isoDate(values.endsAt),
          deliveryMode: values.deliveryMode,
          learningObjectives: rows(text(values, 'learningObjectives')),
          teachingNotes: text(values, 'teachingNotes'),
          pblEnabled: enabled(values, 'pblEnabled'),
          pblProjects: rows(text(values, 'pblProjects')).map((line) => {
            const [name = '', deliverable = '', description = ''] = line
              .split('|')
              .map((part) => part.trim());
            return { name, deliverable, description };
          }),
          sceneRules: rows(text(values, 'sceneRules')).map((line) => {
            const [sceneId = '', name = '', condition = 'always', value = ''] = line
              .split('|')
              .map((part) => part.trim());
            return { sceneId, name, condition, value };
          }),
          courseResources: rows(text(values, 'courseResources')).map((line) => {
            const [title = '', type = 'link', url = ''] = line
              .split('|')
              .map((part) => part.trim());
            return { title, type, url };
          }),
          agentSettings: {
            tutorEnabled: enabled(values, 'tutorEnabled'),
            peerEnabled: enabled(values, 'peerEnabled'),
            monitorEnabled: enabled(values, 'monitorEnabled'),
            strategyEnabled: enabled(values, 'strategyEnabled'),
          },
          promptStrategy: {
            version: text(values, 'promptVersion'),
            policy: text(values, 'promptPolicy'),
          },
          gradingPolicy: {
            formativeWeight: number(values, 'formativeWeight'),
            projectWeight: number(values, 'projectWeight'),
            finalWeight: number(values, 'finalWeight'),
          },
          assignmentPolicy: {
            assignmentCount: number(values, 'assignmentCount'),
          ...
          interventionPolicy: {
            strategy: values.interventionStrategy,
            message: text(values, 'interventionMessage'),
          },
              ...
              >
                ...
                <Field label="课程开始时间">
                  <Input
                    name="startsAt"
                    type="datetime-local"
                    defaultValue={localDate(course.startsAt)}
                  />
                </Field>
                <Field label="课程结束时间">
                  <Input
                    name="endsAt"
                    type="datetime-local"
                    defaultValue={localDate(course.endsAt)}
                  />
                </Field>
                <Field label="教学模式">
                  <select
                  ...
                  >
                    ...
                    <option value="blended">混合式</option>
                    <option value="face_to_face">面授</option>
                  </select>
                </Field>
                <Field label="发布状态">
                  <select
                    ...
                    defaultValue={course.publicationStatus}
                  >
                ...
                <Wide label="教学说明">
                  <Textarea name="teachingNotes" rows={4} defaultValue={course.teachingNotes} />
                </Wide>

                <Section title="PBL、场景与资源" />
                <Toggle name="pblEnabled" label="启用 PBL" checked={course.pblEnabled} />
                <div />
                <Wide label="PBL 项目（每行：项目名称 | 成果要求 | 项目说明）">
                  <Textarea
                    name="pblProjects"
                    rows={5}
                    defaultValue={course.pblProjects
                      .map((p) => `${p.name} | ${p.deliverable} | ${p.description}`)
                      .join('\n')}
                  />
                </Wide>
                <Wide label="场景开放条件（每行：场景ID | 名称 | always/date/previous_completed/score | 条件值）">
                  <Textarea
                    name="sceneRules"
                    rows={5}
                    defaultValue={course.sceneRules
                      .map((r) => `${r.sceneId} | ${r.name} | ${r.condition} | ${r.value}`)
                      .join('\n')}
                  />
                </Wide>
                <Wide label="课程资源（每行：标题 | document/video/link/dataset/other | 地址）">
                  <Textarea
                    name="courseResources"
                    rows={5}
                    defaultValue={course.courseResources
                ...
                <Toggle
                  name="tutorEnabled"
                  label="Tutor 智能体"
                  checked={course.agentSettings.tutorEnabled}
                />
                <Toggle
                  name="peerEnabled"
                  label="Peer 智能体"
                  checked={course.agentSettings.peerEnabled}
                />
                <Toggle
                  name="monitorEnabled"
                  label="Monitor 智能体"
                  checked={course.agentSettings.monitorEnabled}
                />
                <Toggle
                  name="strategyEnabled"
                  label="Strategy 智能体"
                  checked={course.agentSettings.strategyEnabled}
                />
                <Field label="提示词策略版本">
                  <Input
                    name="promptVersion"
                    defaultValue={course.promptStrategy.version}
                    required
                  />
                </Field>
                <Wide label="提示词策略">
                  <Textarea
                    name="promptPolicy"
                    rows={6}
                    defaultValue={course.promptStrategy.policy}
                  />
                </Wide>
                ...
                <NumberField
                  ...
                  value={course.gradingPolicy.formativeWeight}
                  min={0}
                  max={100}
                />
                <NumberField
                  label="项目成绩权重（%）"
                  name="projectWeight"
                  value={course.gradingPolicy.projectWeight}
                  min={0}
                  max={100}
                />
                <NumberField
                  label="期末成绩权重（%）"
                  name="finalWeight"
                  value={course.gradingPolicy.finalWeight}
                  min={0}
                  max={100}
                />
                <NumberField
                  ...
                  value={course.assignmentPolicy.assignmentCount}
                  min={0}
                  max={200}
                />
                <NumberField
                  label="每次作业最大尝试次数"
                  name="maxAttempts"
                  value={course.assignmentPolicy.maxAttempts}
                  min={1}
                  max={100}
                />
                ...
                <NumberField
                  label="成绩预警阈值"
                  name="scoreThreshold"
                  value={course.warningPolicy.scoreThreshold}
                  min={0}
                  max={100}
                />
                <NumberField
                  label="未学习天数阈值"
                  name="inactivityDays"
                  value={course.warningPolicy.inactivityDays}
                  min={1}
                  max={365}
                />
                <NumberField
                  ...
                  value={course.warningPolicy.missedAssignments}
                  min={0}
                  max={200}
                />
                <Field label="干预方式">
                  <select
                    ...
                    defaultValue={course.interventionPolicy.strategy}
                  >
                    <option value="notify_student">通知学生</option>
                    <option value="notify_teacher">通知教师</option>
                    <option value="agent_coaching">智能体辅导</option>
                    <option value="manual_follow_up">人工跟进</option>
                  </select>
                </Field>
                <Wide label="干预消息/执行说明">
                  <Textarea
...
function Section({ title }: { title: string }) {
  return <h2 className="border-b pb-2 pt-4 text-lg font-semibold md:col-span-2">{title}</h2>;
}
...
function NumberField({
  ...
  return (
    <Field label={label}>
      <Input name={name} type="number" min={min} max={max} step="1" defaultValue={value} required />
    </Field>
  );
}
