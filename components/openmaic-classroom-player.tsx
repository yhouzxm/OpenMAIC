...
export function OpenMaicClassroomPlayer({ classroomId: providedClassroomId, postgres = true, readOnly = false, focusSceneId, initialSceneId, backHref }: { classroomId?: string; postgres?: boolean; readOnly?: boolean; focusSceneId?: string; initialSceneId?: string; backHref?: string }) {
  ...
  const loadClassroom = useCallback(
    async (isEffectCurrent: () => boolean = () => true) => {
      ...
      if (focusSceneId && isCurrent()) {
        const store = useStageStore.getState();
        const scene = store.scenes.find((item) => item.id === focusSceneId);
        if (!scene) { setError('Configured OpenMAIC scene was not found'); return; }
        store.setScenes([scene]);
        store.setCurrentSceneId(scene.id);
      } else if (initialSceneId && isCurrent()) {
        const store = useStageStore.getState();
        if (store.scenes.some((item) => item.id === initialSceneId)) store.setCurrentSceneId(initialSceneId);
      }
    },
    [classroomId, focusSceneId, initialSceneId, loadFromStorage, postgres, readOnly],
  );
      ...
      <MediaStageProvider value={classroomId}>
        <div className="h-screen flex flex-col overflow-hidden">
          {loading ? (
            ...
            <Stage onRetryOutline={retrySingleOutline} backHref={backHref} />
          )}
        </div>
      </MediaStageProvider>
    </ThemeProvider>
