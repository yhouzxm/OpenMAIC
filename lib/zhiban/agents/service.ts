...
import type { CourseAgentRuntime, InterventionBrief, InterventionTarget, ZhibanAgentRole } from './types';
...
export function decideIntervention(
  ...
  const achievement = numeric(dimensions.achievement);
  const completion = numeric(dimensions.completion);
  const engagement = numeric(dimensions.engagement);
  const collaboration = numeric(dimensions.collaboration);
  const threshold = numeric(warning.scoreThreshold) || 60;
  if (achievement > 0 && achievement < threshold && settings.tutorEnabled)
    return { target: 'tutor', objective: '定位知识卡点并提供分步讲解与一次理解检查', tone: '清晰、耐心、鼓励自主完成' };
  if (completion < 35 && engagement < 35)
    return { target: 'teacher', objective: '请教师核验学习阻碍并决定是否人工跟进', tone: '客观、审慎、仅陈述证据' };
  if (collaboration < 30 && settings.peerEnabled)
    return { target: 'peer', objective: '通过同伴式交流帮助学习者说出下一步行动', tone: '平等、简短、非评判' };
  if (completion < 60 && settings.tutorEnabled)
    return { target: 'tutor', objective: '帮助学习者制定一个可在本次课堂完成的小目标', tone: '具体、支持性、不过度催促' };
  return null;
}
...
export async function evaluateMonitorIntervention(
  ...
  return withZhibanTenant(pool, tenantId, async (client) => {
    ...
    const decision = decideIntervention(
      (row.dimensions as Record<string, unknown> | null) ?? {},
      settings,
      (row.warning_policy as Record<string, unknown>) ?? {},
    );
    ...
    const recent = await client.query(
      ...
      [input.learnerId, input.courseId, decision.target],
    );
    if (recent.rows[0]) return { created: false, suppressed: 'cooldown' as const };
    ...
    const inserted = await client.query<{ id: string }>(
      `INSERT INTO zhiban.intervention_briefs(id,tenant_id,learner_id,course_id,source_event_id,target_role,level,objective,tone,evidence_summary,prohibited_content,max_turns,policy_version,prompt_version,command_id)
       VALUES($1,$2,$3,$4,$5,$6,$6,$7,$8,$9::jsonb,$10::jsonb,4,'monitor-v1',$11,$12)
       ON CONFLICT(tenant_id,command_id) DO NOTHING RETURNING id`,
      [id, tenantId, input.learnerId, input.courseId, input.eventId, decision.target, decision.objective, decision.tone,
        JSON.stringify({ dimensions: row.dimensions ?? {}, evidence: row.evidence_summary ?? {} }), JSON.stringify(forbidden), promptVersion, commandId],
    );
    ...
    await client.query(
      ...
      [randomUUID(), tenantId, id, JSON.stringify({ sourceEventId: input.eventId, policyVersion: 'monitor-v1' })],
    );
...
export async function getCourseAgentRuntime(
  ...
  return withZhibanTenant(pool, principal.tenantId, async (client) => {
    ...
    const settings = (result.rows[0]?.agent_settings ?? { tutorEnabled: true, peerEnabled: false, monitorEnabled: false, strategyEnabled: false }) as AgentFeatureSettings;
    const promptVersion = String((result.rows[0]?.prompt_strategy as Record<string, unknown> | undefined)?.version ?? 'v1');
    const templates = await client.query<{ role_type: 'tutor' | 'peer'; name: string; persona: string; version: string }>(
      `SELECT DISTINCT ON(role_type) role_type,name,persona,version FROM zhiban.agent_role_templates
       ...
       ORDER BY role_type,(course_id=$1) DESC,updated_at DESC`, [courseId],
    );
    ...
    const agents = buildCourseAgentConfigs(courseId, settings, promptVersion).map((agent) => {
      ...
      return template ? { ...agent, name: template.name, persona: `${template.persona}\n提示词模板版本：${template.version}` } : agent;
    });
...
function mapBrief(row: Record<string, unknown>): InterventionBrief {
  return {
    id: String(row.id), courseId: String(row.course_id), targetRole: row.target_role as InterventionTarget,
    level: row.level as InterventionTarget, objective: String(row.objective), tone: String(row.tone),
    evidenceSummary: row.evidence_summary as Record<string, unknown>, prohibitedContent: row.prohibited_content as string[],
    maxTurns: Number(row.max_turns), policyVersion: String(row.policy_version), promptVersion: String(row.prompt_version),
    status: row.status as InterventionBrief['status'], createdAt: new Date(row.created_at as string).toISOString(), expiresAt: new Date(row.expires_at as string).toISOString(),
  };
}

export async function listPendingInterventions(pool: ZhibanDatabasePool, principal: AuthorizedPrincipal, courseId: string) {
  return withZhibanTenant(pool, principal.tenantId, async (client) => {
    await client.query(`UPDATE zhiban.intervention_briefs SET status='expired' WHERE learner_id=$1 AND course_id=$2 AND status='pending' AND expires_at<=now()`, [principal.id, courseId]);
    const result = await client.query<Record<string, unknown>>(
...
export async function respondToIntervention(
  ...
  return withZhibanTenant(pool, principal.tenantId, async (client) => {
    const current = await client.query<Record<string, unknown>>(`SELECT * FROM zhiban.intervention_briefs WHERE id=$1 AND learner_id=$2 AND course_id=$3 FOR UPDATE`, [briefId, principal.id, courseId]);
    ...
    await client.query(`UPDATE zhiban.intervention_briefs SET status=$2,responded_at=now() WHERE id=$1`, [briefId, status]);
    await client.query(
...
export async function recordInterventionOutcome(
  pool: ZhibanDatabasePool,
  principal: AuthorizedPrincipal,
  input: { briefId: string; courseId: string; outcome: 'start' | 'deliver' | 'fail'; latencyMs?: number; error?: string },
) {
  return withZhibanTenant(pool, principal.tenantId, async (client) => {
    ...
    const status = input.outcome === 'start' ? 'running' : input.outcome === 'deliver' ? 'delivered' : 'failed';
    await client.query(
    ...
    await client.query(
      ...
      [randomUUID(), principal.tenantId, input.briefId, principal.id, from, status, JSON.stringify({ latencyMs: input.latencyMs, error: input.error })],
    );
    await client.query(
      ...
      [randomUUID(), principal.tenantId, input.briefId, input.courseId, principal.id, row.target_role,
        input.outcome === 'start' ? 'started' : input.outcome === 'deliver' ? 'succeeded' : 'failed', input.latencyMs ?? null, input.error ? 'runtime_error' : null],
    );
    if (input.outcome !== 'start') {
      ...
      await client.query(
        ...
        [eventId, principal.tenantId, principal.id, input.courseId, sourceId, `agent_intervention_${status}`, JSON.stringify({ briefId: input.briefId, role: row.target_role, latencyMs: input.latencyMs, error: input.error })],
      );
      await client.query(
        ...
        [randomUUID(), principal.tenantId, `profile_rebuild:${principal.id}:${input.courseId}:${sourceId}`, JSON.stringify({ learnerId: principal.id, courseId: input.courseId, sourceEventId: sourceId })],
      );
    }
    return { status };
  });
...
export async function listTeacherInterventions(pool: ZhibanDatabasePool, principal: AuthorizedPrincipal, courseId: string) {
...
export async function manageTeacherIntervention(
  pool: ZhibanDatabasePool,
  principal: AuthorizedPrincipal,
  input: { courseId: string; briefId: string; action: 'escalate' | 'resolve' | 'retry' | 'assign'; note?: string; assignedTo?: string },
) {
  if (!canManageCourse(principal, input.courseId)) throw new Error('Permission denied');
  return withZhibanTenant(pool, principal.tenantId, async (client) => {
    const found = await client.query<{ status: string }>(`SELECT status FROM zhiban.intervention_briefs WHERE id=$1 AND course_id=$2 FOR UPDATE`, [input.briefId, input.courseId]);
    if (!found.rows[0]) throw new Error('Intervention not found');
    const status = input.action === 'resolve' ? 'resolved' : input.action === 'retry' ? 'pending' : input.action === 'escalate' ? 'escalated' : found.rows[0].status;
    await client.query(
    ...
    await client.query(
      ...
      [randomUUID(), principal.tenantId, input.briefId, principal.id, found.rows[0].status, status, JSON.stringify({ action: input.action, note: input.note })],
    );
    return { status };
  });
}

export async function listAgentTemplates(pool: ZhibanDatabasePool, principal: AuthorizedPrincipal, courseId: string) {
  if (!canManageCourse(principal, courseId)) throw new Error('Permission denied');
  return withZhibanTenant(pool, principal.tenantId, async (client) =>
    (await client.query(`SELECT id,role_type,version,name,persona,policy,status,created_at,updated_at FROM zhiban.agent_role_templates WHERE course_id=$1 ORDER BY role_type,created_at DESC`, [courseId])).rows,
  );
}

export async function createAgentTemplate(pool: ZhibanDatabasePool, principal: AuthorizedPrincipal, input: { courseId: string; roleType: ZhibanAgentRole; version: string; name: string; persona: string; policy?: Record<string, unknown>; publish?: boolean }) {
  if (!canManageCourse(principal, input.courseId)) throw new Error('Permission denied');
  return withZhibanTenant(pool, principal.tenantId, async (client) => {
    if (input.publish) await client.query(`UPDATE zhiban.agent_role_templates SET status='archived',updated_at=now() WHERE course_id=$1 AND role_type=$2 AND status='active'`, [input.courseId, input.roleType]);
    const result = await client.query(
      ...
      [randomUUID(), principal.tenantId, input.courseId, input.roleType, input.version, input.name, input.persona, JSON.stringify(input.policy ?? {}), input.publish ? 'active' : 'draft', principal.id],
    );
    return result.rows[0];
  });
}

export async function publishAgentTemplate(pool: ZhibanDatabasePool, principal: AuthorizedPrincipal, courseId: string, templateId: string) {
  if (!canManageCourse(principal, courseId)) throw new Error('Permission denied');
  return withZhibanTenant(pool, principal.tenantId, async (client) => {
    const found = await client.query<{ role_type: string }>(`SELECT role_type FROM zhiban.agent_role_templates WHERE id=$1 AND course_id=$2 FOR UPDATE`, [templateId, courseId]);
    if (!found.rows[0]) throw new Error('Template not found');
    await client.query(`UPDATE zhiban.agent_role_templates SET status='archived',updated_at=now() WHERE course_id=$1 AND role_type=$2 AND status='active'`, [courseId, found.rows[0].role_type]);
    await client.query(`UPDATE zhiban.agent_role_templates SET status='active',updated_at=now() WHERE id=$1`, [templateId]);
