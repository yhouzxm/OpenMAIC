...
const actions = [
  ...
  phase: 'inspection',
  ...(action === 'SUBMIT_DIAGNOSIS' ? { value: 'S2_OUTPUT_ABNORMAL' } : {}),
}));
const context = buildTrainingContext({
  ...
  snapshot: {
    ...
    workpiece: { detectedByS1: true, detectedByS2: true, position: { x: 1, y: 1, z: 1 } },
    sensors: { s1: false, s2: true, s2Powered: true, s2Output: true, s2Faulty: false },
    motor: false,
    conveyor: false,
    cylinder: false,
    plc: { inputs: { s1: false, s2: true }, outputs: { motor: false, cylinder: false } },
    training: {
...
function session(
  overrides: Partial<PersistedVirtualLabSession & { userId: string; name: string }> = {},
) {
  return {
    id: 'session-1',
    userId: 'student-1',
    ...
    verificationPassed: true,
    ...overrides,
  } as PersistedVirtualLabSession & { userId: string; name: string };
}
...
describe('Virtual Lab persistence data logic', () => {
  ...
  it('calculates course metrics and error-pattern percentages from completed records', () => {
    const weak = session({
      ...
      assessment: {
        ...assessment,
        errorPatterns: ['SKIP_PLC_INSPECTION', 'SKIP_OUTPUT_MEASUREMENT'],
      },
    });
    ...
    expect(analytics.errorPatterns).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: 'SKIP_PLC_INSPECTION', percent: 50 }),
      ]),
    );
    expect(analytics.interventions).toContain('建议加强现场设备状态与 PLC I/O 对应关系训练。');
  });

  it('uses all enrolled learners as the completion and error-pattern denominator', () => {
    const weak = session({
      id: 'weak',
      userId: 'student-2',
      assessment: { ...assessment, errorPatterns: ['SKIP_PLC_INSPECTION'] },
    });
    const analytics = buildTeacherVirtualLabAnalytics(
      [session(), weak],
    ...
    expect(analytics.errorPatterns).toContainEqual({
      code: 'SKIP_PLC_INSPECTION',
      count: 1,
      percent: 33,
    });
  });

  it('uses each completed learner latest valid assessment for class performance averages', () => {
    ...
    const analytics = buildTeacherVirtualLabAnalytics(
      [firstAttempt, latestAttempt, anotherLearner],
      ['student-1', 'student-2', 'student-3'],
    );
